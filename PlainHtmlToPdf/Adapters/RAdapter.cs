...
using PlainHtmlToPdf.Core.Handlers;
...
public abstract class RAdapter
{
    ...
    public RImage GetLoadingImage()
    {
        if (_loadImage == null)
        {
            var stream = typeof(HtmlRendererUtils).Assembly.GetManifestResourceStream("PlainHtmlToPdf.Core.Utils.ImageLoad.png");
            if (stream != null)
    ...
    public RImage GetLoadingFailedImage()
    {
        if (_errorImage == null)
        {
            var stream = typeof(HtmlRendererUtils).Assembly.GetManifestResourceStream("PlainHtmlToPdf.Core.Utils.ImageError.png");
            if (stream != null)
